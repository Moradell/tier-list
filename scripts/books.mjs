...
import { BOOK_COLUMNS, BOOK_TIERS, parseBooksCsv } from '../src/lib/books.js'
...
function assertNoDuplicates(groups) {
  ...
  for (const group of groups) {
    for (const book of group.books) {
      ...
      if (seen.has(id)) {
        throw new Error(`Книга ${id} повторяется в ${seen.get(id)} и ${group.relativePath}`)
      }
...
async function sort() {
  ...
  for (const group of groups) {
    const books = group.books
      ...
      .sort((left, right) => tierOrder.get(left.book.tier) - tierOrder.get(right.book.tier) || left.index - right.index)
      .map(({ book }) => book)
...
try {
  ...
  else throw new Error('Использование: node scripts/books.mjs <validate|sort>')
} catch (error) {
