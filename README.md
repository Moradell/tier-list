...
Новую запись удобнее добавлять интерактивной командой:

```bash
npm run movies:add
# либо сразу передать ссылку
npm run movies:add -- https://www.kinopoisk.ru/film/123456/
```

Команда использует `KINOPOISK_API_KEY` из `.env.local`, сама получает название,
постер, жанры, страны, актёров, режиссёров и длительность, определяет каталог,
а затем отдельно запрашивает пользовательскую оценку от 1 до 10 и подтверждение.
Новая запись становится первой в выбранном каталоге; взаимный порядок уже
существующих записей не меняется.
...
только новые `kp_id` с постером, названием, пользовательской оценкой и датой добавления. Команда
